#Adjuntar variables de entorno
 locals {
  environment_prefix          = "${lookup(local.env.environment_prefix, terraform.workspace, environment_prefix)}"
  environment_name            = "${lookup(local.env.environment_name, terraform.workspace, environment_name)}"
